...
export const CheckTargetModal = ({ hebrew, label, currentTarget, onSave, onClose }: Props) => {
    ...
    <div className="fixed inset-0 z-[60] bg-black/50 flex items-end justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 pt-5 pb-3 shrink-0">
          <div>
            <p className="font-serif text-xl text-sepia-900 dark:text-sepia-50">{hebrew}</p>
            <p className="text-xs text-sepia-500">{label}</p>
          </div>
        ...
        <div className="px-5 pb-2">
          ...
          <p className="text-xs text-sepia-400 text-center mt-2">
            {target === 1
              ? '1 punto esta semana'
              : `Acumulá +${target} puntos esta semana en ${label.toLowerCase()}`}
          </p>
        </div>
